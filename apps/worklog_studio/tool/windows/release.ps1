...
git fetch --tags
if (!(git tag -l $tag)) {
    Write-Host "ERROR: Tag does not exist: $tag — run publish.ps1 first" -ForegroundColor Red
    exit 1
}

if (!(Test-Path $zipPath)) {
    Write-Host "ERROR: ZIP not found: $zipPath — run build.ps1 first" -ForegroundColor Red
    exit 1
}
...
if ($tag -like "*dev*") {
    ...
    Write-Host "Creating PRE-release" -ForegroundColor Yellow
} else {
...
if ([string]::IsNullOrWhiteSpace($prevTag)) {
    Write-Host "No previous tag — using full log"
