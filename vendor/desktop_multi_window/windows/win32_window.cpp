...
Win32Window::MessageHandler(HWND hwnd,
                            ...
                            LPARAM const lparam) noexcept {
  switch (message) {
    // Hide instead of destroy when the user clicks the X button on a
    // secondary window. Without this, WM_CLOSE -> DefWindowProc ->
    // DestroyWindow -> WM_DESTROY frees flutter_controller_ while Win32
    // still has WM_NCDESTROY queued, causing 0xC0000005 in the message pump.
    case WM_CLOSE:
      if (!quit_on_close_) {
